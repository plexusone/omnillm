...
func (p *Provider) CreateChatCompletion(ctx context.Context, req *provider.ChatCompletionRequest) (*provider.ChatCompletionResponse, error) {
	// Convert from unified format to GLM format (OpenAI-compatible)
	glmReq := &Request{
		Model:            req.Model,
	...
	for _, msg := range req.Messages {
		glmReq.Messages = append(glmReq.Messages, Message{
	...
	resp, err := p.client.CreateCompletion(ctx, glmReq)
	if err != nil {
		return nil, err
	}

	if len(resp.Choices) == 0 {
		return nil, errors.New("glm: empty choices in completion response")
	}
...
func (p *Provider) CreateChatCompletionStream(ctx context.Context, req *provider.ChatCompletionRequest) (provider.ChatCompletionStream, error) {
	// Convert from unified format to GLM format
	glmReq := &Request{
		Model:            req.Model,
		MaxTokens:        req.MaxTokens,
		Temperature:      req.Temperature,
	...
	stream, err := p.client.CreateCompletionStream(ctx, glmReq)
	if err != nil {
		return nil, err
	}
